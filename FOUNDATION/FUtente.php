...
class FUtente extends FDatabase{
    ...
    private static $values = '(:id, :Nome, :Cognome, :Email, :Password, :Stato, :Admin)';
    ...
    public static function bind($stmt, EUtente $Utente)
    {
        ...
        $stmt->bindValue(':Stato', $Utente->getStato(), PDO::PARAM_STR);
    ...
    public static function loadByField($parametri = array(), $ordinamento = '', $limite = ''){
        ...
        if(($result != null) && ($rows_number == 1)){
            $utente = new EUtente($result['Nome'],$result['Cognome'], $result['id'],  $result['Password'], $result['Stato'],$result['Email'],$result['Admin']);
         }
        else{
            if(($result != null) && ($rows_number > 1)){
                $utente = array();
                for($i = 0; $i < count($result) ;$i++){
                    $utente[] = new EUtente($result[$i]['Nome'],$result[$i]['Cognome'], $result[$i]['id'],  $result[$i]['Password'], $result[$i]['Stato'],$result[$i]['Email'],$result[$i]['Admin']);
